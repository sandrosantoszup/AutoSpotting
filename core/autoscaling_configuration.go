...
func (a *autoScalingGroup) loadGP2ConversionThreshold() {
	...
	if tagValue == nil {
		logger.Printf("Couldn't load the GP2ConversionThreshold from tag %v, using the default value of %v\n", GP2ConversionThresholdTag, a.config.GP2ConversionThreshold)
		return
	}

	logger.Printf("Loaded GP2ConversionThreshold value %v from tag %v\n", *tagValue, GP2ConversionThresholdTag)

	threshold, err := strconv.Atoi(*tagValue)
	if err != nil {
		logger.Printf("Error parsing %v qs integer: %s\n", *tagValue, err.Error())
		return
	}
